...
output$IPsummary = renderText({
  ...
  output$GOSRMsummary = renderText({

    gosrmscore = omega.gen.SS.rm(
      as.numeric(input$GOSRMdfmod),
      as.numeric(input$GOSRMdferr),
      as.numeric(input$GOMssmod),
      as.numeric(input$GOSRMss1),
      as.numeric(input$GOSRMsstot),
      as.numeric(input$GOSRMmssub),
      as.numeric(input$GOSRMj2),
      as.numeric(input$GOSRMF),
      as.numeric(input$GOSRMalpha))

    HTML(paste("<b>Definition:</b> ", eta, "<p/>",
               "<b>Effect Size:</b> ", apa_var(gosrmscore, input$GOSRMalpha, type = "omegag"), "<p/>", #effect size
               "<b>Interpretation:</b> ", checkzero(gosrmscore$omegalow, gosrmscore$omegahigh), "<p/>", #effect size interpretation
               "<b>Test Statistic:</b> ", apa_stat(gosrmscore, "F"), "<p/>", #test stats
               "<b>Interpretation:</b> ", checkp(gosrmscore$p, input$GOSRMalpha), #test interpretation
               sep = ""))

  }) #close generalized omega rm
